...
from verde.brackets import COMMA_PRIORITY, DOT_PRIORITY, BracketTracker
from verde.mode import Mode, Preview
...
from verde.strings import str_width
...
class Indentation(Enum):
    ...
    def num_spaces(self, mode: Mode) -> int:
        if mode.is_verde and self == Indentation.SCOPE:
            return mode.verde_indentation
        else:
            # Both verde and black use 4 spaces for continuations.
            return 4
...
class EmptyLineTracker:
    ...
    def _maybe_empty_lines_for_class_or_def(  # noqa: C901
        ...
        if len(self.previous_line.depth) < len(current_line.depth) and (
            (not self.mode.is_verde and self.previous_line.is_class)
