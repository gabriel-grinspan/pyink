...
from _verde_version import version as __version__
from verde.cache import Cache
from verde.comments import normalize_fmt_off
from verde.const import (
    DEFAULT_EXCLUDES,
    DEFAULT_INCLUDES,
    DEFAULT_LINE_LENGTH,
    STDIN_PLACEHOLDER,
)
from verde.files import (
    find_project_root,
    find_pyproject_toml,
...
from verde.handle_ipynb_magics import (
    PYTHON_CELL_MAGICS,
    TRANSFORMED_MAGICS,
...
from verde.linegen import LN, LineGenerator, transform_line
from verde.lines import EmptyLineTracker, LinesBlock
from verde.mode import FUTURE_FLAG_TO_FEATURE, VERSION_TO_FEATURES, Feature
from verde.mode import Mode as Mode  # re-exported
from verde.mode import QuoteStyle, TargetVersion, supports_feature
from verde.nodes import (
    STARS,
    is_number_token,
...
from verde.output import color_diff, diff, dump_to_file, err, ipynb_diff, out
from verde.parsing import InvalidInput  # noqa F401
from verde.parsing import lib2to3_parse, parse_ast, stringify_ast
from verde import ink
from verde import ink_adjusted_lines
from verde.report import Changed, NothingChanged, Report
...
@click.option(
    ...
    default=True,
    help=(
        "Enable the Verde formatting mode. Disabling it should behave the same as"
        " Black."
    ),
)
@click.option(
...
@click.option(
    ...
    default=None,
    help=(
        "Number of parallel workers [default: VERDE_NUM_WORKERS environment variable "
...
def main(  # noqa: C901
    ...
    preview: bool,
    verde: bool,
    verde_indentation: str,
    verde_lines: Optional[Sequence[str]],
    verde_use_majority_quotes: bool,
    quiet: bool,
    verbose: bool,
    ...
    if target_version:
        ...
        versions = set()
    verde_indentation = 2 if verde_indentation == "2" else 4
    mode = Mode(
        ...
        is_verde=verde,
        verde_indentation=verde_indentation,
        quote_style=(
            QuoteStyle.MAJORITY if verde_use_majority_quotes else QuoteStyle.SINGLE
        ),
    )

    lines: Optional[List[Tuple[int, int]]] = None
    if verde_lines is not None:
        lines = []
        for lines_str in verde_lines:
            parts = lines_str.split("-")
            if len(parts) != 2:
                err(
                    "Incorrect --verde-lines format, expect 'START-END', found"
                    f" {lines_str!r}"
                )
                ctx.exit(1)
            try:
    ...
    if code is not None:
        ...
        if len(sources) == 1:
            ...
            from verde.concurrency import reformat_many
...
def format_str(
    ...
    *,
    ...
    >>> import verde
    >>> print(verde.format_str("def f(arg:str='')->None:...", mode=verde.Mode()))
    def f(arg: str = "") -> None:
        ...
    ...
    ...     mode=verde.Mode(
    ...       target_versions={verde.TargetVersion.PY36},
    ...       line_length=10,
